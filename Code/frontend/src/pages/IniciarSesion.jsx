...
export default function IniciarSesion() {
  ...
  const [recoveryPasswordFocused, setRecoveryPasswordFocused] = useState(false);
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      ...
      if (!isPasswordValid(recoveryPassword)) {
        throw new Error("La contraseña debe tener al menos 8 caracteres, un número y un carácter especial (,*.!?)");
      }
                ...
                <div className="urban-login__group" style={{ position: "relative" }}>
                  ...
                  <div style={{ display: "flex", position: "relative" }}>
                    <input
                      ...
                      onFocus={() => setRecoveryPasswordFocused(true)}
                      onBlur={() => setRecoveryPasswordFocused(false)}
                      required
                  ...
                  <ul className={`urban-register__password-requirements${recoveryPasswordFocused ? ' visible' : ''}`} style= {{zIndex: 1000}}>
                    <li className={recoveryPassword.length >= 8 ? "met" : "unmet"}>
                      <span className="icon">{recoveryPassword.length >= 8 ? "✓" : "✕"}</span> Al menos 8 caracteres
                    </li>
                    <li className={/\d/.test(recoveryPassword) ? "met" : "unmet"}>
                      <span className="icon">{/\d/.test(recoveryPassword) ? "✓" : "✕"}</span> Un número
                    </li>
                    <li className={/[,*.!?.:;\-_{}|()/¿¡#@$%&"'€+]/.test(recoveryPassword) ? "met" : "unmet"}>
                      <span className="icon">{/[,*.!?.:;\-_{}|()/¿¡#@$%&"'€+]/.test(recoveryPassword) ? "✓" : "✕"}</span> Un carácter especial (,*.!?)
                    </li>
                  </ul>
                </div>
                <div className="urban-login__actions" style={{ marginTop: "1.5rem", display: "flex", gap: "10px" }}>
